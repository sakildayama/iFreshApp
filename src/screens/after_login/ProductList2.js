...
import { ProductCategories2, FlatListData2 } from '../../constants/datas'
const ProductList2 = ({ navigation }) => {
  const [productListData, setProductListData] = useState(FlatListData2)
  const [activeCategory, setActiveCategory] = useState("All Category")

  const changecategory = (category) => {
    if (category == 'All Category') {
      setProductListData(FlatListData2);
    } else if (category == 'Dry Fruits') {
      let filter_Product = FlatListData2.filter((data) => {
